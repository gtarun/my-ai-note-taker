...
import {
  appendSheetRow,
  ensureLayerSpreadsheet,
  type ExtractionFieldPayload,
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const ensuredSheet = await ensureLayerSpreadsheet({
      accessToken,
    ...
    const appendResult = await appendSheetRow({
      accessToken,
      spreadsheetId: ensuredSheet.spreadsheetId,
      sheetTitle: ensuredSheet.sheetTitle,
      fields: body.fields,
      values: body.values ?? {},
    });
    ...
    return jsonResponse({
      ...ensuredSheet,
      rowRange: appendResult.updates?.updatedRange ?? '',
    });
  } catch (error) {
    return jsonResponse(
      { error: error instanceof Error ? error.message : 'Unable to append the Google Sheets row.' },
      500
    );
  }
});
